...
out = os.path.join(root, 'dsfovea2')
ebl = os.path.join(os.getenv("HOME"), 'eblhuman')
bin = os.path.join(ebl, 'bin')

train = ['set00', 'set01', 'set02', 'set03', 'set04']
val = ['set05']
h = 128#66 #72 #76 #126 #126 #78 #114 #78 #128 # 160 # 128 # target height
w = 78 #42 #40 #78 #54 #36 # 58 #38 # 64 # 96 # 76 # target width
...
maxbg = 2000 # initial number of negatives (per set)
...
for e in all:
    ...
    cmd = dscompile + dataroot + ' -type pascalbg -annotations ' + aroot \
      ...
      + dims + ' ' + ppresize + fovea + ' -maxperclass ' + str(nbg) \
    ...
    cmd = dscompile + outbg + ' ' + precision \
          ...
          + str(w) + 'x' + str(chans) + fovea + ' -nopp ' + debug
    if os.system(cmd) != 0: sys.exit(-1)
