...
void ebl_basic_test::test_state_copy() {
  ...
  dseed(32);
  idx_aloop3(xx,a,double,xd,a.b[0],double,xdd,a.bb[0],double){
    *xx = drand(2);
    *xd = drand(2);
    *xdd = drand(2);
  }
  ...
  CPPUNIT_ASSERT(0 == idx_sqrdist(a.b[0],b.b[0]));
  CPPUNIT_ASSERT(0 == idx_sqrdist(a.bb[0],b.bb[0]));

  idx_addc(a,1.,a);
  idx_addc(a.b[0],2.,a.b[0]);
  idx_addc(a.bb[0],3.,a.bb[0]);
  CPPUNIT_ASSERT(a.footprint() == idx_sqrdist(a,b));
  CPPUNIT_ASSERT(4*a.b[0].footprint() == idx_sqrdist(a.b[0],b.b[0]));
  CPPUNIT_ASSERT(9*a.bb[0].footprint() == idx_sqrdist(a.bb[0],b.bb[0]));
}
...
inline void test_conv_cuda(int inx, int iny, int infeat, int outfeat,
  ...
  c1.forget(fp); // randomize bbparameters if there are any
  c2.fprop(in,out2); // just to resize states
  c2.forget(fp); // randomize bbparameters if there are any
...
void ebl_basic_test::test_sqrt_power_module_float() {
  ...
  CPPUNIT_ASSERT_DOUBLES_EQUAL(0.3536, in.b[0].get(0), 0.0001);
  CPPUNIT_ASSERT_DOUBLES_EQUAL(0.125, in.bb[0].get(0), 0.0001);
  // sqrt
  { bbparameter<T> p(10000);
    power_module<T> m(.5);
    state<T> in(2, 11, 11), out;
