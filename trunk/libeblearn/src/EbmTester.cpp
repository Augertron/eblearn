...
void ModuleTester::test_jacobian(module_1_1<state_idx,state_idx> *module, state_idx *in, state_idx *out)
{
...
void ModuleTester::test_jacobian_param(parameter *p, module_1_1<state_idx,state_idx>* module, state_idx *in, state_idx *out)
{
	forget_param_linear fp(2,0);
...
void ModuleTester::get_jacobian_fprop(module_1_1<state_idx,state_idx> *module, state_idx *in, state_idx *out,Idx<double>& jac)
{
...
void ModuleTester::get_jacobian_fprop_param(parameter *p, module_1_1<state_idx,state_idx> *module, state_idx *in, state_idx *out,Idx<double>& jac)
{
	state_idx souta = out->make_copy(); //f(x-small)
	state_idx soutb = out->make_copy(); //f(x+small)
	double small = 1e-6;
...
void ModuleTester::get_jacobian_bprop(module_1_1<state_idx,state_idx> *module, state_idx *in, state_idx *out,Idx<double>& jac)
{
	jac.resize(in->size(),out->size());
	idx_clear(jac);
	int cnt = 0;
...
void ModuleTester::get_jacobian_bprop_param(parameter *p, module_1_1<state_idx,state_idx> *module, state_idx *in, state_idx *out,Idx<double>& jac)
{
}
...
void ModuleTester::report_err(Idx<double>& a, Idx<double>& b, const char* msg)
{
	...
	// report results
	ss << "Max " << msg << " distance";
	fprintf(this->out,"%-40s = %-15g %15s\n",ss.str().c_str(),maxdist,((maxdist<this->acc_thres)?"OK":"NOT OK"));
	ss.str("");
	ss << "Total " << msg << "distance";
	fprintf(this->out,"%-40s = %-15g %15s\n",ss.str().c_str(),totdist,((totdist<this->acc_thres)?"OK":"NOT OK"));
	fflush(this->out);
}
